...
def preprocess():
    ...
    src_file = os.path.join(base_dir, "datasets", "rag vector-db", "ICMR_STW_Vol_2_PTB_EPTB.pdf")
    tgt_file = os.path.join(base_dir, "data", "processed", "clinical_triage_kb", "icmr_stw_ptb_eptb.jsonl")
    ...
    for idx, chunk in enumerate(chunks, start=1):
        record = {
            "id": f"icmr_stw_ptb_eptb_{idx:04d}",
            "text": chunk,
            "metadata": {
                "source_file": "ICMR_STW_Vol_2_PTB_EPTB.pdf",
                "target_collection": "clinical_triage_kb",
                "jurisdiction_level": "CENTRAL",
                "state_code": "ALL",
                "scheme_name": "ICMR_STW",
                "doc_type": "tb_protocol",
                "chunk_id": idx
            }
        }
